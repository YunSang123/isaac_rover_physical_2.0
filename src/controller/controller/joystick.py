...
class Physical_osr(Node):
    ...
    def __init__(self):
        ...
        self.subscription = self.create_subscription(Twist, '/joy/joy', self.exteroception_cb,1000)
        self.subscription = self.create_subscription(Twist, '/joy/joy', self.odometry_cb,1000)
    
        # TODO : 변경해야함
        self.publisher_ = self.create_publisher(Twist, '/osr/osr_vel', 1000)
        timer_period = 0.1  # seconds
        self.timer = self.create_timer(timer_period, self.timer_callback)   # 5Hz 주기로 실행
        self.goal_dist = 1
    ...
    def exteroception_cb(self, msg):
        # self.sparse에 값 저장하는 내용
        # self.dense에 값 저장하는 내용
        pass

    def odometry_cb(self, msg):
        # self.goal_dist에 distance 저장하는 내용
        # self.goal_heading에 heading 저장하는 내용
        pass
